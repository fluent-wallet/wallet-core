...
describe('addChain', () => {
  test('add a chain', async () => {
    const chainData = await addChain(global.database, ConfluxMainnet);
    expect(chainData).toEqual(
      expect.objectContaining(ConfluxMainnet),
    );
  ...
  test('add a chain with a single endpoint', async () => {
    ...
    const chainData = await addChain(global.database, convertEndpointsToEndpoint(ConfluxTestnet));
  ...
  test('add a chain without chainId', async () => {
    const chainData = await addChain(global.database, SolanaMainnet);
    expect(chainData).toEqual(
      expect.objectContaining(SolanaMainnet),
    );
